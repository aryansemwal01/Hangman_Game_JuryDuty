...
class HangmanGameGUI:
    def _init_(self, master):
        ...
        self.gif_photo = None  # Attribute to store the image reference
    ...
    def end_game(self, win):
        pygame.mixer.init()
        pygame.mixer.music.load("win_sound.wav" if win else "lose_sound.wav")
        pygame.mixer.music.play(-1)

        gif_path = "win.gif" if win else "lose.gif"
        gif_image = Image.open(gif_path)
        self.gif_photo = ImageTk.PhotoImage(gif_image)

        end_game_window = tk.Toplevel(self.master)
        end_game_window.title("Game Over")

        label = tk.Label(end_game_window, text="You win!" if win else "Game Over!")
        label.pack()

        # Display the image
        image_label = tk.Label(end_game_window, image=self.gif_photo)
        image_label.image = self.gif_photo  # Keep a reference to avoid garbage collection
        image_label.pack()
